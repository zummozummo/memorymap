import express from "express";
import { json } from "body-parser";
import cookieSession from "cookie-session";

import "express-async-errors";

import { createBlockRouter } from "./routes/new";
import { indexBlockRouter } from "./routes/index";
import { showBlockRouter } from "./routes/show";
import { updateBlockRouter } from "./routes/update";
import { errorHandler, NotFoundError, currentUser } from "@sgtickets/common";
const app = express();
//to make sure that express is behind a proxy engress_Enginx
//and it should trust traffic even though its coming from that proxy
app.set("trust proxy", true);

app.use(json());
app.use(
  cookieSession({
    signed: false,
    secure: process.env.NODE_ENV !== "test",
  })
);
app.use(currentUser);
app.use(createBlockRouter);
app.use(showBlockRouter);
app.use(indexBlockRouter);
app.use(updateBlockRouter);
app.all("*", (req, res, next) => {
  throw new NotFoundError();
});

app.use(errorHandler);

//this is named export that why {}
export { app };
